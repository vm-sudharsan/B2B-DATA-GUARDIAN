...
class AIInferenceEngine:
    ...
    def load_models(self):
        ...
        try:
            ...
            sentence_path = MODELS_DIR / "correction_sentence_model.pkl"
            if sentence_path.exists():
                self.sentence_model = joblib.load(sentence_path)
            ...
            if references_path.exists():
                ...
                for name in self.references.keys():
                    index_path = MODELS_DIR / f"faiss_{name}.index"
                    if index_path.exists():
                        self.faiss_indexes[name] = faiss.read_index(str(index_path))
                print(f"✓ FAISS Indexes loaded: {list(self.faiss_indexes.keys())}")
    ...
    def detect_duplicate_probability(self, rec1: Dict, rec2: Dict) -> Dict[str, Any]:
        if not self.duplicate_detector:
            ...
            return {"recordA_id": rec1.get("id", 0), "recordB_id": rec2.get("id", 1), "duplicate_probability": 0.0, "explanation": ["Model not loaded, using fallback"]}
        try:
            features = self._extract_duplicate_features(rec1, rec2)
            proba = self.duplicate_detector.predict_proba([features])[0][1]
            feature_names = ["name_similarity", "email_match", "email_username_sim", "email_domain_match", "phone_match", "company_sim", "title_sim"]
            explanation = [f"{name}: {value:.2f}" for name, value in zip(feature_names, features) if value > 0.7] or ["Low similarity across all fields"]
    ...
    def detect_anomaly(self, field: str, value: str) -> Dict[str, Any]:
        if not self.anomaly_detectors or field not in self.anomaly_detectors:
            return {"field": field, "value": value, "anomaly_score": 0.0, "is_suspicious": False, "explanation": ["Anomaly detector not available for this field"]}
        try:
            ...
            if sum(not c.isalnum() and not c.isspace() for c in value) / len(value) > 0.3:
    ...
    def suggest_correction(self, field_type: str, value: str, top_k: int = 3) -> Dict[str, Any]:
        if field_type not in self.faiss_indexes or field_type not in self.references:
            return {"original_value": value, "suggested_value": value, "similarity_confidence": 0.0, "explanation": ["No reference data available"]}
        try:
            query_embedding = self.sentence_model.encode([value], convert_to_numpy=True)
            distances, indices = self.faiss_indexes[field_type].search(query_embedding.astype("float32"), top_k)
            top_idx = indices[0][0]
            top_distance = distances[0][0]
            suggested = self.references[field_type][top_idx]
            similarity = 1.0 / (1.0 + top_distance)
            alternatives = [self.references[field_type][i] for i in indices[0][1:]]
            return {"original_value": value, "suggested_value": suggested, "similarity_confidence": round(float(similarity), 3), "explanation": [f"Closest match: {suggested}", f"Alternatives: {', '.join(alternatives)}"]}
        except Exception as e:
    ...
    def compute_quality_score(self, stats: Dict[str, int]) -> Dict[str, Any]:
        ...
        w = 0.20
        missing_p = (stats.get("missing_fields", 0) / total) * w
        invalid_p = (stats.get("invalid_fields", 0) / total) * w
        anomaly_p = (stats.get("anomalies", 0) / total) * w
        duplicate_p = (stats.get("duplicates", 0) / total) * w
        low_conf_p = (stats.get("low_confidence_fixes", 0) / total) * w
        total_penalty = missing_p + invalid_p + anomaly_p + duplicate_p + low_conf_p
        quality_score = max(0, min(100, 100 - (total_penalty * 100)))
        breakdown = {
            "missing_fields_impact": round(missing_p * 100, 2),
            "invalid_format_impact": round(invalid_p * 100, 2),
            "anomaly_impact": round(anomaly_p * 100, 2),
            "duplicate_impact": round(duplicate_p * 100, 2),
            "low_confidence_impact": round(low_conf_p * 100, 2),
        }
        if quality_score >= 90:
